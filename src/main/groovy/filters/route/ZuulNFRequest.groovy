...
class ZuulNFRequest extends ZuulFilter {
    ...
    Object run() {
        ...
        if (headers.get("Content-Type") != null && headers.get("Content-Type").get(0).contains("x-www-form-urlencoded")) formParams = buildZuulRequestFormParams(request)
        ...
        if (verb.verb().equals("POST") && headers.get("Content-Type").get(0).contains(MediaType.APPLICATION_FORM_URLENCODED)) {
            for (param in formParams) {
                params.add(param.key, param.value[0]);
            }
        }
    ...
    def Map<String, String> buildZuulRequestFormParams(HttpServletRequest request) {

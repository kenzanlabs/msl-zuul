...
class Routing extends ZuulFilter {
    ...
    Object run() {
        ...
        String vip;
        try {
            vip = ((NFRequestContext) RequestContext.currentContext).getRequest().pathInfo.split("/")[1].replace("-", "");
        } catch (Exception e) {
            vip = ""
        } finally {
            if (StringUtils.isEmpty(vip)) vip = defaultClient.get();
        }

        ((NFRequestContext) RequestContext.currentContext).routeVIP = vip
